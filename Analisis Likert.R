...
#Instalar los paquete que vamos a utilizar.
...
# Exploramos la matriz de datos Pisa
names(pisa)
typeof(pisa)
str(pisa)
...
# buscamos los valores perdidos en la matri Pisa
anyNA(pisa)

# numero total de datos perdidos en la matriz de datos
suma(is.na(pisa))

# Se hace la eliminacion de filas con datos perdidos
# y creamos una nueva matriz de datos
pisa2 <-na.omit(pisa)
...
niveles(items24)<-c("Completamente en desacuerdo",
                           "En desacuerdo",
                           "De acuerdo",
...
levels(items24)<-c("Leo si es necesario",
...
#Realiamos un grafico de barras ordenado
plot(l24,group.order = colnames(items24))

#Realizamos un grafico de densidad
...
#Realiamos el agrupamiento de los items
...
#Para realiar el grafico de barras con porcentajes instalamos el 
#paquete plyr
install.packages("plyr")
library(plyr)
